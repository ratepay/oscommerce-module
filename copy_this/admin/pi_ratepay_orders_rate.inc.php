...
if (is_osc231()) {
    ...
    $contents[] = array('align' => 'center', 'text' => tep_draw_button('RatePAY Details', 'document', tep_href_link('pi_ratepay_admin_osc231.php', 'oID=' . $oInfo->orders_id), null) . tep_draw_button('RatePAY Rechnung', 'document', tep_href_link('pi_ratepay_rate_invoice.php', 'oID=' . $oInfo->orders_id), null, $params));
} else {
    array_push($contents, array(
            'align' => 'center',
            'text' => '<a href="' . tep_href_link('pi_ratepay_rate_print_order.php', 'oID=' . $oInfo->orders_id . '&payment=' . $oInfo->payment_method) . '" TARGET="_blank">' . tep_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a> <a href="' . tep_href_link(FILENAME_ORDERS_PACKINGSLIP, 'oID=' . $oInfo->orders_id) . '" TARGET="_blank">' . tep_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a>'
        )
    );
    array_push($contents, array(
            'align' => 'center',
