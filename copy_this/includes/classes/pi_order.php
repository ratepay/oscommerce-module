...
*/
  ...
  class pi_order {
    ...
    function query($order_id) {
      ...
      $shipping_method_query = tep_db_query("select value from " . TABLE_ORDERS_TOTAL . " where orders_id = '" . (int)$order_id . "' and class = 'ot_shipping'");
    ...
    function cart() {
      ...
      if( isset( $_SESSION['pi_coupon'] ) ) {
        require_once( DIR_WS_CLASSES.'discount_coupon.php' );
        $this->coupon = new discount_coupon( $_SESSION['pi_coupon'], $this->delivery );
      ...
      for ($i=0, $n=sizeof($products); $i<$n; $i++) {
        ...
        if( is_object( $this->coupon ) ) {
          $applied_discount = 0;
          $applied_discount = $this->coupon->calculate_discount( $this->products[$index], $valid_products_count );
          if( $applied_discount > 0 ) $valid_products_count++;
          $shown_price = $this->coupon->calculate_shown_price( $applied_discount, $this->products[$index] );
