...
class pi_ratepay_rechnung {
    ...
    function pi_ratepay_rechnung() {
        ...
        $this->title = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_TEXT;
        ...
        $this->ust = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_UST;
        //End custom
    ...
    function selection() {
        ...
        if ($customer['customers_telephone'] == '' || !preg_match('/^[0-9\/\-+() ]*$/', $customer['customers_telephone'])) {
            $fieldsBool = true;
            $fields[] = array('title' => 'Telefon', 'field' => tep_draw_input_field('pi_phone', ''));
        }

        if ($customer['customers_dob'] == '0000-00-00 00:00:00') {
            $fieldsBool = true;
            $fields[] = array('title' => 'Geburtstag', 'field' => tep_draw_input_field('pi_birthdate', '') . " " . PI_RATEPAY_RECHNUNG_VIEW_PAYMENT_BIRTHDATE_FORMAT);
        }

        if (($order->customer['company'] != '' || $order->billing['company'] != '')) {
            $fieldsBool = true;
            $fields[] = array('title' => PI_RATEPAY_RECHNUNG_VIEW_PAYMENT_VATID, 'field' => tep_draw_input_field('pi_vatid', ''));
        }

        if ($fieldsBool) {
            $display = array('id' => $this->code, 'module' => tep_image(DIR_WS_IMAGES . '/ratepay_rechnung_rgb.png'), 'fields' => $fields);
        } else {
            $display = array('id' => $this->code, 'module' => tep_image(DIR_WS_IMAGES . '/ratepay_rechnung_rgb.png'));
        }
    ...
    function pre_confirmation_check() {
        ...
        if(isset($_POST['conditions'])){
            ...
            if ($this->getOrderTotal($order) < $this->min || $this->getOrderTotal($order) > $this->max) {
                $errorStr = urlencode(sprintf(PI_RATEPAY_RECHNUNG_ERROR_AMOUNT, $this->min, $this->max));
                tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT,'payment_error=' . $this->code . '&error=' . $errorStr, 'SSL'));
            }
            ...
            if (isset($_POST['pi_phone']) && isset($_POST['pi_birthdate'])) {
                ...
                if ($_POST['pi_phone'] != '' && preg_match('/^[0-9\/\-+() ]*$/', $_POST['pi_phone'])) {
                    $successFon = true;
                    $customerId = $_SESSION['customer_id'];
                    tep_db_query("update " . TABLE_CUSTOMERS . " set customers_telephone = '" . tep_db_prepare_input($_POST['pi_phone']) . "' WHERE customers_id ='" . tep_db_input($customerId) . "' ");
                }

                $dob = tep_db_prepare_input($_POST['pi_birthdate']);
            ...
            if(($order->customer['company'] != '' || $order->billing['company'] != '') && $_POST['pi_vatid'] == ''){
		$errorStr = urlencode(PI_RATEPAY_ERROR_VATID_ERROR);
                tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $this->code,'SSL') . '&error=' . $errorStr);
            } else if(($order->customer['company'] != '' || $order->billing['company'] != '') && $_POST['pi_vatid'] != ''){
		$_SESSION['pi']['vatid'] = $_POST['pi_vatid'];
            ...
            if ($inputNeededFon == true && $inputNeededBirthdate == true) {
                ...
                if ($successFon == true && $successDate == true) {
                    if($this->is_osc231()) {
                        tep_redirect(tep_href_link("checkout_pi_ratepay_rechnung_terms_osc231.php", "coupon=" . $coupon, 'SSL',true));
                    } else {
                        tep_redirect(tep_href_link("checkout_pi_ratepay_rechnung_terms.php?osCsid=" . tep_session_id() . "&coupon=" . $coupon,'', 'SSL'));
                    }
                } else {
    ...
    function after_process() {
        ...
        if ($response) {
            ...
            if ((string) $response->head->processing->status->attributes()->code == "OK" && (string) $response->head->processing->result->attributes()->code == "400") {
                ...
                $sql = "INSERT INTO pi_ratepay_rechnung_orders (order_number, transaction_id, transaction_short_id, descriptor, first_name, last_name, dob, gender, vat_id, coupon_code)
                ...
                if (isset($_SESSION['pi']['discount'])) {
                    foreach ($_SESSION['pi']['discount'] as $discount) {
                        if (number_format($discount['amount'], 2, ".", "") != 0) {
                            $sql = "select count(*) as nr from pi_ratepay_rechnung_orderdetails where article_number like 'DISCOUNT%'";
                            $query = tep_db_query($sql);
                            $nr = tep_db_fetch_array($query);
                            $sql = "INSERT INTO pi_ratepay_rechnung_orderdetails (order_number,article_number, real_article_number, article_name,ordered,article_netUnitPrice, tax)
    ...
    function check() {
        if (!isset($this->_check)) {
            $check_query = tep_db_query("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_STATUS'");
    ...
    function install() {
        global $language;
	@include(DIR_FS_CATALOG . DIR_WS_LANGUAGES . $language . '/modules/payment/pi_ratepay_rechnung.php');

        $check_query = tep_db_query("SHOW TABLES LIKE 'pi_ratepay_rechnung_orders'");
        ...
        $check_query = tep_db_query("SHOW TABLES LIKE 'pi_ratepay_rechnung_orderdetails'");
        if (tep_db_num_rows($check_query) == 0) {
            tep_db_query(
        ...
        $check_query = tep_db_query("SHOW TABLES LIKE 'pi_ratepay_rechnung_history'");
        if (tep_db_num_rows($check_query) == 0) {
            tep_db_query(
        ...
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_STATUS_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_STATUS', 'True', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_STATUS_DESC . "', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PROFILE_ID_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PROFILE_ID', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PROFILE_ID_DESC . "', '6', '3', NOW())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SECURITY_CODE_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SECURITY_CODE', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SECURITY_CODE_DESC . "', '6', '3', NOW())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SANDBOX_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SANDBOX', 'False', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SANDBOX_DESC . "', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_LOGS_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_LOGS', 'False', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_LOGS_DESC . "', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_GTC_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_GTC', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_GTC_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PRIVACY_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PRIVACY', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PRIVACY_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_PRIVACY_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_PRIVACY', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_PRIVACY_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_NAME_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_NAME', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_NAME_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_BANK_NAME_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_BANK_NAME', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_BANK_NAME_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_CODE_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_CODE', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_CODE_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ACCOUNT_NR_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ACCOUNT_NR', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ACCOUNT_NR_DESC . "', '6', '0', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SWIFT_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SWIFT', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SWIFT_DESC . "', '6', '0', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_IBAN_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_IBAN', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_IBAN_DESC . "', '6', '0', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MAX_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MAX', '0', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MAX_DESC . "', '6', '0', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MIN_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MIN', '0', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MIN_DESC . "', '6', '0', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_EXTRA_FIELD_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_EXTRA_FIELD', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_EXTRA_FIELD_DESC . "', '6', '0', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_ORDER_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_ORDER', '0', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_ORDER_DESC . "', '6', '0', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ZONE_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ZONE', '0', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ZONE_DESC . "', '6', '2', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ORDER_STATUS_ID_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ORDER_STATUS_ID', '0', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ORDER_STATUS_ID_DESC . "', '6', '0', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ALLOWED_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ALLOWED', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ALLOWED_DESC . "', '6', '0', now())" );

        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_OWNER_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_OWNER', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_OWNER_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_HR_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_HR', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_HR_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FON_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FON', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FON_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FAX_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FAX', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FAX_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_PLZ_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_PLZ', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_PLZ_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_STREET_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_STREET', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_STREET_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_COURT_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_COURT', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_COURT_DESC . "', '6', '3', now())" );
        tep_db_query ( "INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_UST_TITLE . "', 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_UST', '', '" . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_UST_DESC . "', '6', '3', now())" );
    }
    ...
    function remove() {
        ...
        //tep_db_query("DROP TABLE `pi_ratepay_log`, `pi_ratepay_rechnung_history`, `pi_ratepay_rechnung_orderdetails`, `pi_ratepay_rechnung_orders`");
    }
    ...
    function paymentRequest($order) {
        ...
        if ($_SESSION['customers_status']['customers_status_show_price_tax'] == 0 && $_SESSION['customers_status']['customers_status_add_tax_ot'] == 1) {
            $total = ($order->info['total'] + $order->info['tax']) * $order->info['currency_value'];
	}
	else {
        ...
        if ($order->customer['company'] != '') {
            $customer->addChild('company-name', $this->removeSpecialChars($order->customer['company']));
    ...
    public function getOrderTotal($order)
    {
        ...
        foreach ($this->getDiscounts($order) as $discount) {
            $total = $total + $discount['amount'] + $discount['tax'];
        }
